...
func SetupRoutes(app *fiber.App) {
	...
	deleteAccountHandler := user.NewDeleteAccountHandler(postgresRepo, slogLogger, mailersendService)
	...
	forgotPasswordHandler := user.NewForgotPasswordHandler(postgresRepo, mailersendService, jwtTokenService, slogLogger, validator)
	resetPasswordHandler := user.NewResetPasswordHandler(postgresRepo, jwtTokenService, slogLogger, validator)
	...
	createTodoHandler := todo.NewCreateTodoHandler(postgresRepo, redisClient, slogLogger)
	...
	app.Get("/healthcheck", Handle(healthcheckHandler, slogLogger))
	app.Use(contextMiddleware)

	adminApp := app.Group("/admin", middlewareManager.AuthMiddleware, middlewareManager.AdminMiddleware)

	app.Post("/signup", Handle(signupHandler, slogLogger))
	app.Post("/login", Handle(loginHandler, slogLogger))

	usersPublicApp := app.Group("/users")
	usersPublicApp.Post("/forgot-password", Handle(forgotPasswordHandler, slogLogger))
	usersPublicApp.Post("/reset-password", Handle(resetPasswordHandler, slogLogger))
	usersPublicApp.Post("/verify-email", Handle(verifyEmailHandler, slogLogger))

	usersApp := app.Group("/users", middlewareManager.AuthMiddleware)
	usersApp.Get("/profile", Handle(getCurrentUserHandler, slogLogger))
	usersApp.Delete("/account", Handle(deleteAccountHandler, slogLogger))
	usersApp.Patch("/account", Handle(updateFullNameHandler, slogLogger))
	usersApp.Patch("/password", Handle(updatePasswordHandler, slogLogger))
	usersApp.Post("/send-verification-email", Handle(sendVerificationEmailHandler, slogLogger))

	usersAdminApp := adminApp.Group("/users")
	usersAdminApp.Get("/", Handle(getUsersHandler, slogLogger))
	usersAdminApp.Get("/:id", Handle(getUserHandler, slogLogger))

	todosApp := app.Group("/todos", middlewareManager.AuthMiddleware)
	todosApp.Post("/", Handle(createTodoHandler, slogLogger))
	todosApp.Get("/:id", Handle(getTodoByIdHandler, slogLogger))
	todosApp.Get("/", Handle(getTodosHandler, slogLogger))
	todosApp.Put("/:id", Handle(updateTodoHandler, slogLogger))
	todosApp.Delete("/:id", Handle(deleteTodoHandler, slogLogger))
	todosApp.Patch("/:id", Handle(toggleCompletedTodoHandler, slogLogger))
